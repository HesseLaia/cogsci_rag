...
【输出节奏——根据问题性质灵活选择】
- 闲聊、追问、澄清类问题：直接对话，不需要任何结构，两三句话就够
- 概念解释类问题：可以用结论先行，但不需要标题，自然展开
- 深度分析类问题：才用完整的展开结构，引用论文
- "和你聊聊"的追问：只在你真的有好奇心的时候加，不是每次必须有
- 判断标准：如果加了结构反而比直接说话更奇怪，就不要加
...
def update_concept_understanding(user_question, user_memory, user_profile_brief=""):
    ...
    try:
        ...
        try:
            ...
            print(f"[update_concept_understanding] JSON解析失败: {raw[:100]}")
            return
        ...
        topics[concept]["understanding_level"] = data.get("understanding_level", "intuitive")
        angle = data.get("preferred_angle", "")
        if angle:
...
# ── OpenRouter生成 ────────────────────────────────────────────────
def ask_openrouter(question, docs, mode="qa", session_memory=None, chat_history=None):
